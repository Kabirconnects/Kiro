...
const store = new Store({
  ...
  defaults: {
    ...
    apiKey: '',
    model: '',
    character: { skin: 'violet' },
    profile: { name: '' },
    project: { folder: null },
    screenAssist: { enabled: false, intervalMs: 30000 }
  }
});
...
function createCatWindow() {
  ...
  catWindow = new BrowserWindow({
    width: CAT_SIZE, height: CAT_SIZE, x: width - CAT_SIZE - 40, y: height - CAT_SIZE - 40,
...
function createPanelWindow() {
  ...
  panelWindow = new BrowserWindow({
    width: panelW, height: panelH, x, y, minWidth: 340, minHeight: 420,
...
app.whenReady().then(() => {
  createCatWindow();
  createTray();
  app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createCatWindow(); });
});
app.on('window-all-closed', () => {});

ipcMain.on('cat:toggle-panel', () => {
  if (panelWindow) panelWindow.isVisible() ? panelWindow.hide() : panelWindow.show();
...
async function captureScreenImage() {
  ...
  return { imageData, sourceName: source.name || 'Screen', width: source.thumbnail.getSize().width, height: source.thumbnail.getSize().height };
}

ipcMain.handle('screen:capture', async () => {
  const win = panelWindow || catWindow;
  const { response } = await dialog.showMessageBox(win, {
    type: 'question', buttons: ['Cancel', 'Capture screen'], defaultId: 1, cancelId: 0,
    message: 'Let Kiro see your screen?',
    detail: 'Kiro will capture the current desktop image once and send it to your selected AI provider only when you ask for screen help.'
  });
  if (response !== 1) return { canceled: true };
  try { return await captureScreenImage(); } catch (err) { return { error: err.message || String(err) }; }
});

// Used only while the user has explicitly enabled Auto Screen Assist in Settings.
...
ipcMain.handle('project:listFiles', async () => {
  ...
  async function walk(dir, depth) {
    ...
    for (const entry of entries) {
      ...
      const full = path.join(dir, entry.name), rel = path.relative(folder, full);
      if (entry.isDirectory()) await walk(full, depth + 1); else results.push(rel);
    }
  }
  await walk(folder, 0);
...
ipcMain.handle('project:readFile', async (event, relativePath) => {
  ...
  try { return { content: await fs.readFile(withinProjectFolder(folder, relativePath), 'utf-8') }; }
...
ipcMain.handle('project:writeFile', async (event, { relativePath, content }) => {
  ...
  const win = panelWindow || catWindow;
  const { response } = await dialog.showMessageBox(win, {
    type: 'question', buttons: ['Cancel', `Overwrite ${relativePath}`], defaultId: 1, cancelId: 0,
    message: `Let Kiro write to "${relativePath}"?`, detail: 'This will overwrite the file with the AI-generated content shown in the panel.'
  });
  if (response !== 1) return { error: 'Write cancelled.' };
  try { await fs.writeFile(withinProjectFolder(folder, relativePath), content, 'utf-8'); return { ok: true }; }
  catch (err) { return { error: err.message }; }
});
  ...
  try {
    if (provider === 'anthropic') {
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: model || 'claude-sonnet-4-6', max_tokens: 2000, system: systemPrompt, messages: [{ role: 'user', content }] })
      });
    ...
    const openAiContent = imageData ? [
      { type: 'text', text: fullUserPrompt }, { type: 'image_url', image_url: { url: imageData } }
    ] : fullUserPrompt;
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model: model || 'gpt-4o-mini', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: openAiContent }] })
    });
    const data = await res.json();
