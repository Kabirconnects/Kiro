...
const store = new Store({ name: 'kiro-settings', defaults: { provider: 'openai', apiKey: '', model: '', character: { skin: 'violet' }, profile: { name: '' }, project: { folder: null }, screenAssist: { enabled: false, intervalMs: 30000 } } });
...
function createTray() {
  ...
  tray.setContextMenu(Menu.buildFromTemplate([{ label: 'Open Kiro Panel', click: () => createPanelWindow() }, { label: 'Show/Hide Cat', click: () => { if (!catWindow) return; catWindow.isVisible() ? catWindow.hide() : catWindow.show(); } }, { type: 'separator' }, { label: 'Quit Kiro', click: () => app.quit() }]));
}
...
ipcMain.handle('screen:autoCapture', async () => { if (!store.get('screenAssist.enabled')) return { error: 'Auto Screen Assist is disabled.' }; try { return await captureScreenImage(); } catch (err) { return { error: err.message || String(err) }; } });

const IGNORE_DIRS = new Set(['node_modules', '.git', 'release', 'dist', '.next', '.venv']);
function withinProjectFolder(root, relativePath) { const resolved = path.resolve(root, relativePath), normalizedRoot = path.resolve(root) + path.sep; if (resolved + path.sep !== normalizedRoot && !resolved.startsWith(normalizedRoot)) throw new Error('Path escapes the permitted project folder.'); return resolved; }
ipcMain.handle('project:chooseFolder', async () => { const result = await dialog.showOpenDialog(panelWindow || catWindow, { properties: ['openDirectory'] }); if (result.canceled || !result.filePaths[0]) return store.get('project'); store.set('project', { folder: result.filePaths[0] }); return store.get('project'); });
ipcMain.handle('project:listFiles', async () => { const folder = store.get('project.folder'); if (!folder) return { error: 'No project folder selected yet.' }; const results = []; async function walk(dir, depth) { if (depth > 4 || results.length > 500) return; let entries; try { entries = await fs.readdir(dir, { withFileTypes: true }); } catch { return; } for (const entry of entries) { if (IGNORE_DIRS.has(entry.name) || entry.name.startsWith('.')) continue; const full = path.join(dir, entry.name), rel = path.relative(folder, full); if (entry.isDirectory()) await walk(full, depth + 1); else results.push(rel); } } await walk(folder, 0); return { files: results, folder }; });
ipcMain.handle('project:readFile', async (event, relativePath) => { const folder = store.get('project.folder'); if (!folder) return { error: 'No project folder selected.' }; try { return { content: await fs.readFile(withinProjectFolder(folder, relativePath), 'utf-8') }; } catch (err) { return { error: err.message }; } });
async function writeProjectFile(relativePath, content) { const folder = store.get('project.folder'); if (!folder) return { error: 'No project folder selected.' }; await fs.writeFile(withinProjectFolder(folder, relativePath), content, 'utf-8'); return { ok: true }; }
ipcMain.handle('project:writeFile', async (event, { relativePath, content }) => { const folder = store.get('project.folder'); if (!folder) return { error: 'No project folder selected.' }; const { response } = await dialog.showMessageBox(panelWindow || catWindow, { type: 'question', buttons: ['Cancel', `Overwrite ${relativePath}`], defaultId: 1, cancelId: 0, message: `Let Kiro write to "${relativePath}"?`, detail: 'This will overwrite the file with the AI-generated content shown in the panel.' }); if (response !== 1) return { error: 'Write cancelled.' }; try { return await writeProjectFile(relativePath, content); } catch (err) { return { error: err.message }; } });
ipcMain.handle('project:applyFix', async (event, { relativePath, content }) => { try { return await writeProjectFile(relativePath, content); } catch (err) { return { error: err.message }; } });
